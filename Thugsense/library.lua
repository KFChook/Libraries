-- Thugsense (Mobile Supported)
--[[
    Made by samet (Modified for Mobile Support)
    
    Mobile features:
    - Auto-detects mobile device
    - Adds top bar button with "Open" to toggle menu
    - Swipe gestures for dropdowns (optional)
    - Mobile-friendly touch interactions
    
    Documentation (same as original plus mobile):
    function Library:EnableMobileSupport(Config: table)
        Config = {
            OpenButtonText = "Open",  -- Text on the open button
            OpenButtonPosition = "TopBar",  -- "TopBar" or "Corner"
            EnableSwipeGestures = true  -- Enable swipe gestures for dropdowns
        }
    )
]]
...
local Library do
    ...
    local TouchService = game:GetService("TouchEnabled") and game:GetService("UserInputService") or nil
    ...
    Library = {
        ...
        -- Mobile specific
        MobileOpenButton = nil,
        MobileMode = false,
        MobileConfig = {
            OpenButtonText = "Open",
            OpenButtonPosition = "TopBar",
    ...
    local Instances = { } do
        ...
        Instances.OnTouch = function(self, Function)
            if not self.Instance then 
                return
            end
            
            local TouchBegan = self:Connect("TouchBegan", Function)
            local MouseClick = self:Connect("MouseButton1Click", Function)
            
            return { TouchBegan, MouseClick }
        end
    end
    ...
    Library.EnableMobileSupport = function(self, Config)
        Config = Config or {}
        self.MobileMode = true
        
        self.MobileConfig.OpenButtonText = Config.OpenButtonText or "Open"
        self.MobileConfig.OpenButtonPosition = Config.OpenButtonPosition or "TopBar"
        self.MobileConfig.EnableSwipeGestures = Config.EnableSwipeGestures ~= false
        
        -- Create the mobile open button
        local buttonPosition
        local buttonAnchor
        
        if self.MobileConfig.OpenButtonPosition == "Corner" then
            buttonPosition = UDim2New(0, 10, 0, 50)
            buttonAnchor = Vector2New(0, 0)
        else -- TopBar by default
            buttonPosition = UDim2New(0.5, -40, 0, 10)
            buttonAnchor = Vector2New(0.5, 0)
        end
        
        self.MobileOpenButton = Instances:Create("TextButton", {
            Parent = self.Holder.Instance,
            FontFace = self.Font,
            TextColor3 = FromRGB(215, 215, 215),
            BorderColor3 = FromRGB(10, 10, 10),
            Text = self.MobileConfig.OpenButtonText,
            Name = "\0",
            Position = buttonPosition,
            AnchorPoint = buttonAnchor,
            Size = UDim2New(0, 80, 0, 35),
            BorderSizePixel = 2,
            BackgroundColor3 = FromRGB(15, 15, 20),
            ZIndex = 1000
        })
        
        self.MobileOpenButton:AddToTheme({BackgroundColor3 = "Background", BorderColor3 = "Border"})
        
        Instances:Create("UIStroke", {
            Parent = self.MobileOpenButton.Instance,
        ...
        self.MobileOpenButton:MakeDraggable()
        
        -- Button hover/touch effects
        self.MobileOpenButton:OnTouch(function()
            self.MobileOpenButton:Tween(nil, {BackgroundColor3 = self.Theme["Hovered Element"]})
            self.MobileOpenButton:ChangeItemTheme({BackgroundColor3 = "Hovered Element"})
            
            task.wait(0.1)
            self.MobileOpenButton:Tween(nil, {BackgroundColor3 = self.Theme["Background"]})
            self.MobileOpenButton:ChangeItemTheme({BackgroundColor3 = "Background"})
        end)
        
        -- Return the button so it can be customized further
        return self.MobileOpenButton
    end
    ...
    Library.CreateColorpicker = function(self, Data)
        ...
        -- Add touch support for colorpicker
        Items["ColorpickerButton"]:Connect("TouchBegan", function()
            Colorpicker:SetOpen(not Colorpicker.IsOpen)
        end)
    ...
    Library.CreateKeybind = function(self, Data)
        ...
        -- Add touch support for keybind (long press for menu)
        local TouchHold = nil
        Items["KeyButton"]:Connect("TouchBegan", function()
            TouchHold = tick()
            task.delay(0.5, function()
                if TouchHold and tick() - TouchHold >= 0.5 then
        ...
        Items["KeyButton"]:Connect("TouchEnded", function()
            if TouchHold and tick() - TouchHold < 0.5 then
                if Keybind.Picking then 
                    return
                end
        ...
        Items["Toggle"]:Connect("TouchBegan", function()
            Keybind.Mode = "Toggle"
            Keybind:SetMode("Toggle")
        end)
        ...
        Items["Always"]:Connect("TouchBegan", function()
            Keybind.Mode = "Always"
            Keybind:SetMode("Always")
        end)
        ...
        Items["Hold"]:Connect("TouchBegan", function()
            Keybind.Mode = "Hold"
            Keybind:SetMode("Hold")
        end)
    ...
    Library.Window = function(self, Data)
        ...
        -- Mobile support: Connect open button if in mobile mode
        if Library.MobileMode and Library.MobileOpenButton then
            Library.MobileOpenButton:Connect("MouseButton1Click", function()
                Window:SetOpen(not Window.IsOpen)
            end)
            Library.MobileOpenButton:Connect("TouchBegan", function()
                Window:SetOpen(not Window.IsOpen)
            end)
        end
    ...
    Library.Page = function(self, Data)
        ...
        -- Add touch support for page switching
        Items["Inactive"]:Connect("TouchBegan", function()
            for Index, Value in Page.Window.Pages do
                Value:Turn(Value == Page)
            end
        end)
    ...
    Library.Pages.SubPage = function(self, Data)
        ...
        -- Add touch support for subpage switching
        Items["Inactive"]:Connect("TouchBegan", function()
            for Index, Value in SubPage.Window.SubPages do
                Value:Turn(Value == SubPage)
            end
    ...
    Library.Pages.MultiSection = function(self, Data)
        ...
        for Index, Value in MultiSection.Sections do 
            ...
            -- Add touch support for multisection switching
            SubItems["Inactive"]:Connect("TouchBegan", function()
                for Index, Value in MultiSection.SectionContents do
                    Value:Turn(Value == NewSection)
                end
            end)
    ...
    Library.Sections.Button = function(self, Data)
        ...
        -- Add touch support for button
        Items["Button"]:Connect("TouchBegan", function()
            Button:Press()
        end)
    ...
    Library.Sections.Slider = function(self, Data)
        ...
        Items["RealSlider"]:Connect("TouchBegan", function(Input)
            Slider.Sliding = true

            local TouchPos = Input.Position

            local SizeX = (TouchPos.X - Items["RealSlider"].Instance.AbsolutePosition.X) / Items["RealSlider"].Instance.AbsoluteSize.X
            local Value = ((Slider.Max - Slider.Min) * SizeX) + Slider.Min
        ...
        Library:Connect(UserInputService.InputChanged, function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseMovement and Slider.Sliding then
                ...
                Slider:Set(Value)
            elseif Input.UserInputType == Enum.UserInputType.Touch and Slider.Sliding then
                local TouchPos = Input.Position
    ...
    Library.Sections.Dropdown = function(self, Data)
        ...
        function Dropdown:Add(Option)
            ...
            OptionButton:Connect("TouchBegan", function()
                OptionData:Set()
            end)
        ...
        -- Add touch support for dropdown open/close
        Items["Open"]:Connect("TouchBegan", function()
            Dropdown:SetOpen(not Dropdown.IsOpen)
        end)
    ...
    Library.Sections.Listbox = function(self, Data)
        ...
        function Listbox:Add(Option)
            ...
            -- Add touch support for listbox options
            OptionButton:Connect("TouchBegan", function()
                OptionData:Set()
            end)
