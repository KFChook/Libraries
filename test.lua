		...
		function library:window(properties)
			...
			-- main window
				local main_window = library:panel({
					name = properties and properties.name or "Kfc.Hook | ", 
...
task.spawn(function()
    while task.wait(1) do 
        ...
        watermark.change_text("Kfc.Hook | Free | Beta | FPS: " .. fps .. " | Ping: " .. ping .. "ms | " .. username .. " ")
    end 
end)
			...
			-- init 
				...
				function cfg.set(input)
					...
					if cfg.name and KEYBIND_ELEMENT and KEYBIND_ELEMENT.Parent then 
						...
						if stroke then stroke.Transparency = 0 stroke.Color = themes.preset.text_outline end
						local text = tostring(cfg.key) ~= "Enums" and (keys[cfg.key] or tostring(cfg.key):gsub("Enum.", "")) or nil
						local __text = text and (tostring(text):gsub("KeyCode.", ""):gsub("UserInputType.", ""))
